...
@torch.no_grad()
def get_training_rays_flatten(rgb_tr_ori, images_opacity_ori, train_poses, HW, Ks, ndc, inverse_y, flip_x, flip_y):
    ...
    opacity_tr = torch.zeros([N,1], device=DEVICE)
    rays_o_tr = torch.zeros_like(rgb_tr)
    rays_d_tr = torch.zeros_like(rgb_tr)
    viewdirs_tr = torch.zeros_like(rgb_tr)
    imsz = []
    top = 0
    for c2w, img, img_opacity, (H, W), K in zip(train_poses, rgb_tr_ori, images_opacity_ori, HW, Ks):
        ...
        opacity_tr[top:top+n].copy_(img_opacity.flatten(0,1).unsqueeze(-1))
    ...
    return rgb_tr, opacity_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz
