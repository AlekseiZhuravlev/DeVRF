...
def load_everything(args, cfg):
    ...
    kept_keys = {
        'hwf', 'HW', 'Ks', 'near', 'far', 'images_opacity',
    ...
    if data_dict['images_opacity'] is not None:
        data_dict['images_opacity'] = torch.FloatTensor(data_dict['images_opacity'], device='cpu')
    return data_dict
...
def scene_rep_reconstruction(args, cfg, cfg_model, cfg_train, xyz_min, xyz_max, data_dict, stage,
    ...
    if abs(cfg_model.world_bound_scale - 1) > 1e-9:
        ...
        xyz_min -= xyz_shift
        xyz_max += xyz_shift
    HW, Ks, near, far, i_train, i_val, i_test, poses, render_poses, images, images_opacity = [
    ...
    def gather_training_rays():
        ...
        if data_dict['irregular_shape']:
            ...
            if images_opacity is not None:
                images_opacity_ori = images_opacity[i_train].to('cpu' if cfg.data.load2gpu_on_the_fly else device)
        ...
        if cfg_train.ray_sampler == 'in_maskcache':
            ...
            rgb_tr, opacity_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz = dvgo.get_training_rays_flatten(
                rgb_tr_ori=rgb_tr_ori,
                images_opacity_ori=images_opacity_ori,
                train_poses=poses[i_train],
        ...
        return rgb_tr, opacity_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, batch_index_sampler

    rgb_tr, opacity_tr, rays_o_tr, rays_d_tr, viewdirs_tr, imsz, batch_index_sampler = gather_training_rays()
    ...
    for global_step in trange(1 + start, 1 + cfg_train.N_iters):
        ...
        if cfg_train.ray_sampler in ['flatten', 'in_maskcache']:
            ...
            target = rgb_tr[sel_i]
            if opacity_tr is not None:
                opa_tr = opacity_tr[sel_i]
        ...
        if cfg_train.weight_alphainv_last > 0:
            loss_opa = torch.mean((opa_tr.squeeze() - render_result['alphainv_last']).pow(2))
            loss += cfg_train.weight_alphainv_last * loss_opa
...
if __name__ == '__main__':
    ...
    if args.render_train:
        ...
        rgbs, depths = render_viewpoints(
            ...
            **render_viewpoints_kwargs)
        imageio.mimwrite(os.path.join(testsavedir, 'video.rgb.mp4'), utils.to8b(rgbs), fps=1, quality=8)
        imageio.mimwrite(os.path.join(testsavedir, 'video.depth.mp4'), utils.to8b(1 - depths / np.max(depths)), fps=1,
                         quality=8)
